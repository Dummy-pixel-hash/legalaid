...
function EvidenceClient({ caseId }: { caseId: string }) {
	...
	const progressPct = items.length
		? Math.round((have / items.length) * 100)
		: 0;
	...
	return (
		<div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
			...
			<div className="mt-6 rounded-lg border border-line bg-surface p-4">
				...
				{items.length > 0 && items.every((i) => i.status !== "unset") && (
				...
				<div className="mt-3 grid gap-3 sm:grid-cols-2">
					<label className="block">
						...
						<input
							...
							className="mt-1 min-h-11 w-full rounded-md border border-line bg-background px-3 py-2 text-xs text-ink placeholder:text-ink-50 focus:border-accent-strong focus:outline-none focus:ring-2 focus:ring-accent-strong/20"
						/>
					</label>
					<label className="block">
						<span className="text-xs font-medium text-ink-50">
			...
			<div className="mt-10 border-t border-line pt-6">
				<Button
					size="lg"
					onClick={() => router.push(`/case/${caseId}/next-steps`)}
				>
					{t("evidenceNext")}
